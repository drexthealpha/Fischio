...
pub fn cpi_validate_stat<'info>(
    ...
    let (pid, ret) = get_return_data().ok_or(WagerError::MissingOracleReturn)?;
    require_keys_eq!(pid, TXORACLE_ID, WagerError::MissingOracleReturn);
    require!(ret.len() == 1, WagerError::MissingOracleReturn);
    Ok(ret[0] == 1)
}
