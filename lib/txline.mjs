...
/**
 * What to say when the feed refuses the request.
 *
 * Without this, someone who clones the repo and runs the CLI gets
 * `/api/fixtures/snapshot?startEpochDay=20653 -> 401` and no idea what to do about it. The feed is
 * licensed, so credentials cannot ship with the source, which makes a missing-credential 401 the
 * single most likely first experience anyone else will have with this code. It deserves a real
 * answer rather than a status code.
 *
 * `guest/start` is not a way out: it answers 401 itself when called unauthenticated, because access
 * begins with an on-chain subscribe transaction rather than a free endpoint.
 */
function unauthorizedMessage(path, creds) {
  const missing = !creds?.jwt || !creds?.apiToken;
  if (missing) {
    return [
      `TxLINE refused ${path} because this process has no feed credentials.`,
      ``,
      `The feed is licensed, so credentials are not in this repository. Two ways forward:`,
      ``,
      `  1. Read from a running fischio instance instead, which needs no credentials of your own:`,
      `       curl http://78.154.103.18:14777/live`,
      `       curl http://78.154.103.18:14777/api/markets`,
      ``,
      `  2. Use your own TxLINE access. Set both, then re-run:`,
      `       TXLINE_JWT=...  TXLINE_API_TOKEN=...`,
      `     or place them in local/credentials.json as { "jwt": "...", "apiToken": "..." }`,
      ``,
      `Every fischio command that reads odds, scores or fixtures needs these. The programs on`,
      `devnet do not: they can be read with any Solana explorer or RPC, with no credentials at all.`,
    ].join("\n");
  }
  return `TxLINE refused ${path} with 401. The credentials are present but rejected, which usually means the guest JWT has expired. Mint a fresh one and retry.`;
}
  ...
  async function getJson(path, { auth = true, attempts = 3 } = {}) {
    ...
    for (let i = 0; i < attempts; i++) {
      try {
        ...
        // A 401 is almost always a missing credential, and the bare status tells the reader
        // nothing about what to do next. Explain it once, here, rather than leaving every caller
        // to guess.
        if (r.status === 401) throw new Error(unauthorizedMessage(path, creds));
